...
export function analyzeText(str: string) {
    ...
    if (timeMatches?.length) {
        ...
        if (structuredTimes.find(time => !time.zone) && structuredTimes.find(time => !!time.zone)) {
            const defaultZone = structuredTimes.find(time => time.zone)
            if (defaultZone) {
                structuredTimes.map(time => time.zone ? time.zone : time.zone = defaultZone.zone)
            }
        }
...
export function getConversionText(dateTime: DateTime, str: string) {
    return (
        "`" + `${str}` + "`" + ` in your time is <t:${dateTime.toSeconds()}:t>`
    );
}
...
export function getVerboseTime(time: TimeStructure) {
    ...
    return `${convertTo24hr(time.hour, meridiem || inferredMeridiem)}${time.minute
        }`;
}
