...
import { analyzeText, getVerboseTime, inferTimezone, getConversionText  } from '../utils/'

const discordClient = new Client({ intents: [IntentsBitField.Flags.Guilds, IntentsBitField.Flags.MessageContent, IntentsBitField.Flags.GuildMessages] })
        ...
        analyzedText.forEach(item => {
            try {
                ...
                const {
                    smallName,
                    timezone,
                } = inferTimezone(item, user)
                if (!timezone) return
                const dt = DateTime.fromFormat(vTime, 'hhmm', { zone: timezone })
                message += getConversionText(
                    dt,
                    `${item.hour}:${item.minute}${item.meridiem ? item.meridiem : ''} ${smallName?.toUpperCase()}`
                )
                if (message) {
                    message += '\n'
                }
